...
async fn title_and_summary_for(client: &Client, cfg: &Config, text: &str) -> Result<DocSummary> {
    ...
    let content = chat(client, cfg, ChatRequest {
        ...
        messages: vec![
            ChatMessage {
                role: "system".into(),
                content: "You are a thorough summarizer. Return a JSON object with a short title (5-8 words, no trailing punctuation) and a comprehensive Markdown summary in the `summary` field. The summary MUST be a multi-line string with multiple paragraphs and ## section headers for each topic covered. Cover all topics, decisions, and details — do not truncate or omit anything significant.".into(),
            },
