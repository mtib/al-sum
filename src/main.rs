...
impl Config {
    ...
    fn al_get(&self, client: &Client, path: &str) -> RequestBuilder {
        client
            .get(format!("{}{}", self.al_url, path))
            .header("Authorization", format!("Bearer {}", self.al_psk))
    }
}
...
fn format_time(ts: f64) -> String {
    use chrono::TimeZone;
    let dt = chrono::Local.timestamp_opt(ts as i64, 0).single().unwrap_or_default();
...
async fn fetch_documents(client: &Client, cfg: &Config) -> Result<Vec<Document>> {
    cfg.al_get(client, "/documents?limit=100")
        .send()
        .await?
        .error_for_status()?
...
async fn fetch_document(client: &Client, cfg: &Config, id: &str) -> Result<DocumentDetail> {
    cfg.al_get(client, &format!("/document/{}", id))
        .send()
        .await?
        .error_for_status()?
        .json::<DocumentDetail>()
...
async fn complete(client: &Client, cfg: &Config, system: &str, user: &str) -> Result<String> {
    let resp = client
        ...
        .json::<ChatResponse>()
        .await?;
    resp.choices.into_iter().next().map(|c| c.message.content).context("no choices in response")
}
...
async fn summary_for(client: &Client, cfg: &Config, text: &str) -> Result<String> {
    let raw = complete(
        client, cfg,
        "You are a thorough summarizer. Write a Markdown summary of the transcript. Start directly with a ## section header — do not write any introduction. Use one ## header per topic, bullet points for details under each header, and **bold** for key terms. Cover every topic and decision. Do not truncate.",
        &format!("Summarize this transcript:\n\n{}", text),
    ).await?;
    let rest = raw.trim_start();
    Ok(if rest.starts_with("# ") {
        rest.find('\n').map(|i| rest[i + 1..].trim_start()).unwrap_or(rest).to_string()
    } else {
        rest.to_string()
    })
}
...
async fn title_and_summary_for(client: &Client, cfg: &Config, text: &str) -> Result<DocSummary> {
    let (title, summary) = tokio::try_join!(title_for(client, cfg, text), summary_for(client, cfg, text))?;
...
async fn cmd_list(client: &Client, cfg: &Config) -> Result<()> {
    ...
    for doc in docs {
        let snippet = doc.snippet.as_deref().unwrap_or("");
        let snippet = if snippet.len() > 80 { format!("{}…", &snippet[..80]) } else { snippet.to_string() };
        println!("[{}] {} → {} {}", doc.doc_id, format_time(doc.started_at), format_time(doc.ended_at), snippet);
    }
    Ok(())
}
...
async fn cmd_summarize(client: &Client, cfg: &Config, doc_id: Option<String>) -> Result<()> {
    match doc_id {
        Some(id) => {
            ...
            let ds = title_and_summary_for(client, cfg, &entries_text(&detail.entries)).await?;
            println!("# {}\n\n_{} → {}_\n\n{}", ds.title, format_time(detail.started_at), format_time(detail.ended_at), ds.summary);
        }
        None => {
            let docs = fetch_documents(client, cfg).await?;
            if docs.is_empty() {
            ...
            for doc in docs {
                ...
                let title = title_for(client, cfg, &entries_text(&detail.entries)).await?;
...
async fn cmd_show(client: &Client, cfg: &Config, doc_id: &str, times: bool, source: bool) -> Result<()> {
    ...
    for entry in &detail.entries {
        ...
        if times { prefix.push_str(&format!("[{} → {}] ", format_time(entry.started_at), format_time(entry.ended_at))); }
        if source { prefix.push_str(&format!("[{}] ", entry.source)); }
...
async fn cmd_search(client: &Client, cfg: &Config, query: &str, limit: u32) -> Result<()> {
    let hits = cfg.al_get(client, "/search/hybrid")
    ...
    for h in hits {
        ...
        println!("[{}] {} → {}{}", h.doc_id, format_time(h.started_at), format_time(h.ended_at), score);
        println!("  {}", h.snippet.unwrap_or_default());
        println!();
    }
    Ok(())
}
